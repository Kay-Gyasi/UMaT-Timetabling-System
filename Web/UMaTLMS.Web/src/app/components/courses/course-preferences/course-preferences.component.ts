...
import {CourseResponse} from "../../../models/responses/course-response";
...
@Component({
  selector: 'app-course-preferences',
  templateUrl: './course-preferences.component.html',
  styleUrls: ['./course-preferences.component.css']
})
export class CoursePreferencesComponent {
  ...
  getPreferences(pageNumber:number = 1){
    ...
    this.pages = [];
    this.courseService.getPreferences(this.query).subscribe({
      next: data => {
        if (data === undefined){
  ...
  private getCoursesLookup(){
    this.lookupService.get(LookupType.Courses).subscribe({
      next: data => {
        if (data === undefined) return;
        this.courses = data;
      },
